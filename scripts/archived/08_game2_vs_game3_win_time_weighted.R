#############################################################################
# Badminton: Time-Weighted Game 2 Momentum -> Game 3 Win
#
# PURPOSE:
# This script tests whether Player 1's time-weighted momentum in Game 2
# is associated with whether Player 1 wins Game 3.
#
# DATA AND MODELS:
...
# 00_compute_elo.R.
#
# Only matches that reached a deciding third game are used. The script fits:
...
# Keep completed matches with both a final Game 3 score and point-by-point
# Game 3 information. These are matches that actually reached Game 3.
df <- df %>%
  filter(
...
cat("Matches that went to a deciding Game 3:", nrow(df), "\n")
...
# Calculate Game 2 momentum for every match
results <- df %>%
  rowwise() %>%
  mutate(
    game_2_points = list(parse_score_list(game_2_scores)),
    game_2_time_momentum =
      compute_time_momentum(game_2_points, c_weight)
  ) %>%
  ungroup()

# Determine whether Player 1 won Game 3
results <- results %>%
  separate(
    game_3_score,
    into = c("g3_p1", "g3_p2"),
  ...
  mutate(
    valid_game_3 = !is.na(g3_p1) & !is.na(g3_p2) & g3_p1 != g3_p2,
    p1_won_game_3 =
      ifelse(valid_game_3, as.integer(g3_p1 > g3_p2), NA)
  )

# Keep one row per match. 
# The momentum variable is already Player 1 minus Player 2.
analysis_data <- results %>%
  filter(!is.na(game_2_time_momentum), !is.na(p1_won_game_3)) %>%
  transmute(
    game_2_time_momentum,
    p1_won_game_3,
    elo_difference_100 =
...
# Use the same complete-case sample for both models. This makes the
# uncontrolled and controlled momentum coefficients directly comparable.
...
model_g2_time_uncontrolled <- glm(
  p1_won_game_3 ~ game_2_time_momentum,
  data = model_data,
  family = binomial
)

cat("\n=== UNCONTROLLED MODEL ===\n")
print(summary(model_g2_time_uncontrolled))

model_g2_time_controlled <- glm(
  p1_won_game_3 ~ game_2_time_momentum + elo_difference_100 +
...
cat("\n=== CONTROLLED MODEL ===\n")
print(summary(model_g2_time_controlled))

# Plot for uncontrolled regression model
regression_plot <- ggplot(
  model_data,
  aes(x = game_2_time_momentum, y = p1_won_game_3)
) +
  ...
  labs(
    title = "Game 3 Win Probability Based on Game 2 Momentum",
    x = "Game 2 Time-Weighted Momentum (Player 1 - Player 2)",
    y = "Probability Player 1 Wins Game 3"
  ) +
...
ggsave(
  filename = paste0(
    ...
    "_game2_time_weighted_momentum_vs_game3_win.png"
  ),
